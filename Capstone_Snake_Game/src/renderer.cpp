...
void Renderer::Render(Snake const snake, SDL_Point const &food, bool *wall, bool *poisoned) {
  ...
  // Render snake's body
  // Colour of snake when it  gets poisoned it renders a greenish body
  if(*poisoned){
...
void Renderer::SetPauseTitle(){
  string title{"[GAME IS PAUSED] press the ESC key to resume game play !"};
  SDL_SetWindowTitle(sdl_window, title.c_str());
}